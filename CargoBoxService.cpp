#include "CargoBoxService.h"
#include "common.h"
#include <string>
#include <iostream>
#include <thread>
#include <mstcpip.h>  // 关键头文件
#pragma comment(lib, "Ws2_32.lib")  // 链接库

static int parseIndex = 0;
static std::vector<uint8_t> recvData(128);
static std::vector<uint8_t> recvDataLast(128);

// 全局变量
static SOCKET client = INVALID_SOCKET;
static std::string Ip = "192.168.144.42"; // 必须使用std::string，不然C#传过来的ip后半部分会乱码
static int Port = 8519;
static bool IsConned = false;

static uint8_t CARGOBOX_ENABLE = 0X12; // 压缩机使能，1字节，0：disable，1：enable
static uint8_t CARGOBOX_ACCELERATE = 0X13; // 压缩机转速提高，1字节：空
static uint8_t CARGOBOX_DECELERATE = 0X14; // 压缩机转速降低，1字节：空
/* 压缩机状态返回，200ms返回一次，11字节
* byte0: 压缩机状态：0和1都是待机，5是正常，6是错误
* byte1: 转速高8位
* byte2: 转速低8位
* byte3~byte4: 当前温度，高位在前，需除以10显示1位小数
* byte5~byte6: 当前电压，高位在前，单位mV
* byte7~byte8: 当前电流，高位在前，单位mA
* byte9~byte10: 错误码，高位在前
*/
static uint8_t CARGOBOX_STATE = 0X15;

static std::vector<CargoBoxServiceStateCallback> cargoBoxStateCallbacks;

// 清理函数
void CargoBoxService_Cleanup() {
    if (client != INVALID_SOCKET) closesocket(client);
    CleanupWinsock();
}

// 设置IP
void CargoBoxService_SetIp(const char* ip) {
    Ip = std::string(ip);
}

static void resetBuffer() {
    std::cerr << "清空recvData: " << std::endl;
    recvData.clear();
    std::cerr << "重置recvData: " << std::endl;
    recvData.resize(128, 0); // 初始化128字节缓冲区
}

static bool parseByte(uint8_t b) {
    switch (parseIndex) {
    case 0: // Header检查
        if (b != 0x8D) {
            parseIndex = 0;
            resetBuffer();
            return false;
        }
        recvData[0] = b;
        parseIndex++;
        return false;

    case 1: // Length字段
        recvData[1] = b;
        parseIndex++;
        return false;

    case 2: // MsgID字段
        recvData[2] = b;
        parseIndex++;
        return false;

    default: { // 数据负载
        const int expectedLength = static_cast<int>(recvData[1]) + 4;
        // 动态扩容（如果需要）
        if (parseIndex >= recvData.size()) {
            recvData.resize(parseIndex + 16); // 每次扩展16字节
        }
        recvData[parseIndex] = b;
        parseIndex++;

        if (parseIndex >= expectedLength) {
            // 复制有效数据段
            recvDataLast.assign(
                recvData.begin(),
                recvData.begin() + expectedLength
            );

            parseIndex = 0;
            resetBuffer();
            return true;
        }
        return false;
    }
    }
}

// 数据接收
static void dataReceive() {
    try {
        std::cerr << CargoBoxService_IsConnected() << std::endl;
        while (CargoBoxService_IsConnected())
        {
            uint8_t recvBuffer[1024];
            const int bytesReceived = recv(client,
                reinterpret_cast<char*>(recvBuffer),
                sizeof(recvBuffer), 0);

            if (bytesReceived == SOCKET_ERROR) {
                const int error = WSAGetLastError();
                if (error == WSAEWOULDBLOCK) continue;
                throw std::runtime_error("接收失败，错误码: " + std::to_string(error));
            }

            if (bytesReceived == 0) {
                continue;
            }
            // 处理每个字节
            for (int i = 0; i < bytesReceived; ++i) {
                if (parseByte(recvBuffer[i])) {
                    // 打印获取到的数据
                    printHex(recvDataLast);
                    for (CargoBoxServiceStateCallback cargoBoxStateCallback : cargoBoxStateCallbacks) {
                        if (recvDataLast[0] != 0x8D) {
                            continue;
                        }
                        if (recvDataLast[2] == CARGOBOX_STATE) {
                            cargoBoxStateCallback(recvDataLast.data(), recvDataLast.size());
                        }
                    }
                }
            }
        }
    }
    catch (const std::exception& e) {
        std::cerr << "清洗水枪消息接收错误: " << e.what() << std::endl;
        CargoBoxService_DisConnected();
    }
}

// 连接
bool CargoBoxService_Connection() {
    if (IsConned) return true;

    client = connection(Ip, Port);
    if (client == INVALID_SOCKET) {
        IsConned = false;
        return !(client == INVALID_SOCKET);
    }

    IsConned = true;
    std::thread t(dataReceive);
    t.detach();
    return true;
}

// 断开连接
void CargoBoxService_DisConnected() {
    if (!IsConned) return;
    closesocket(client);
    IsConned = false;
}

// 检查连接状态
bool CargoBoxService_IsConnected() {
    return IsConned && client != INVALID_SOCKET;
}

// 发送数据
void CargoBoxService_SendData(const char* data, int length) {
    if (!CargoBoxService_IsConnected()) return;

    int result = send(client, data, length, 0);
    if (result == SOCKET_ERROR) {
        // 获取错误码并处理
        int error = WSAGetLastError();
        std::cerr << "清洗水枪消息发送失败，错误码: " << error << std::endl;
        CargoBoxService_DisConnected();
    }
}

// 压缩机使能
void CargoBoxService_SetEnable(bool isEnable) {
    Msg msg;
    msg.SetMsgId(CARGOBOX_ENABLE);
    std::vector<uint8_t> payload(1);
    payload[0] = isEnable ? 1 : 0;
    msg.SetPayload(payload);
    CargoBoxService_SendData(reinterpret_cast<const char*>(msg.GetMsg().data()), msg.length());
}

// 提高功率
void CargoBoxService_IncreasePower() {
    Msg msg;
    msg.SetMsgId(CARGOBOX_ACCELERATE);
    std::vector<uint8_t> payload(1);
    msg.SetPayload(payload);
    CargoBoxService_SendData(reinterpret_cast<const char*>(msg.GetMsg().data()), msg.length());
}

// 降低功率
void CargoBoxService_ReducePower() {
    Msg msg;
    msg.SetMsgId(CARGOBOX_DECELERATE);
    std::vector<uint8_t> payload(4);
    msg.SetPayload(payload);
    CargoBoxService_SendData(reinterpret_cast<const char*>(msg.GetMsg().data()), msg.length());
}

// 注册消防水枪状态回调函数
void CargoBoxService_RegisterCallback(CargoBoxServiceStateCallback callback)
{
    cargoBoxStateCallbacks.push_back(callback);
}